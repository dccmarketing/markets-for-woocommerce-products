...
class Markets_For_Woocommerce_Products_Sanitize {
	...
	public function clean( $data, $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'markets-for-woocommerce-products' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'markets-for-woocommerce-products' ) );
		 ...
		 */
		do_action( 'markets_for_woocommerce_products_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'markets_for_woocommerce_products_post_sanitize', $sanitized );
	...
	/**
	 * Direct cleaner for booleans.
	 *
	 * @param 		bool 		$bool 		boolean
	 * @return 		bool 					Sanitized bool
	 */
	public function clean_bool( $bool ) {

		return $this->clean( $bool, 'checkbox' );

	} // clean_bool()

	/**
	 * Direct cleaner for integers.
	 *
	...
	public function clean_int( $int ) {
		...
		return $this->clean( $int, 'number' );

	} // clean_int()

	/**
	 * Direct cleaner for times or dates.
	 *
	 * @param 		time 		$time 		date or time
	 * @return 		time 					Sanitized date or time
	 */
	public function clean_time( $time ) {

		return $this->clean( $time, 'time' );
